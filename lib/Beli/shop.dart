...
class ProductCache {
  ...
  static void addProducts(List<dynamic> newProducts, int newOffset, bool hasMore) {
...
class _MarketplacePageState extends State<MarketplacePage>
  ...
  List<dynamic> _fullProductList = []; // Always contains all loaded products for section filtering
  ...
  final int _maxDisplayProducts = 100; // Maximum products to display to prevent force close
  ...
  Future<void> _loadProdukForBrand(String brand) async {
    try {
      ...
      final brandProducts = _fullProductList.where((p) {
        ...
        return apiBrand == brand.toUpperCase() || namaProduk.contains(brand.toUpperCase());
      }).toList();
  ...
  Future<void> _loadProduk({bool loadMore = false}) async {
    if (_isLoadingMore && loadMore) return; // Prevent multiple simultaneous loads
    ...
    try {
      if (loadMore) {
        ...
        if (ProductCache.cachedProdukList.length > _produkList.length) {
          final nextBatch = ProductCache.cachedProdukList.skip(_produkList.length).take(20).toList();
          ...
          if (mounted) {
            setState(() {
              ...
              _filteredProduk = _getFilteredList();
              _hasMoreProducts = (ProductCache.cachedProdukList.length > _produkList.length || ProductCache.hasMoreCached) && _produkList.length < _maxDisplayProducts;
        ...
        if (_fullProductList.length > _produkList.length) {
          final nextBatch = _fullProductList.skip(_produkList.length).take(_displayBatchSize).toList();
          await Future.delayed(const Duration(milliseconds: 300)); // Small delay for UX
          ...
          if (mounted) {
            setState(() {
              _produkList.addAll(nextBatch);
              _filteredProduk = _getFilteredList();
              _hasMoreProducts = _fullProductList.length > _produkList.length && _produkList.length < _maxDisplayProducts;
              _isLoadingMore = false;
            });
          }
          return;
              ...
              : _produkList.isEmpty
                  ...
                  : RefreshIndicator(
                      onRefresh: () async {
                        ...
                        try {
                          final result = await ApiService.getProduk(); // Get all products

                          final allProducts = result as List<dynamic>;

                          if (mounted) {
                            setState(() {
                              _produkList = allProducts.take(_displayBatchSize).toList(); // Show only first 20
                              _fullProductList = allProducts; // Store all products for Load More
                              ProductCache.setProducts(allProducts, allProducts.length, false); // Cache all products
                              _filteredProduk = _getFilteredList();
                              _hasMoreProducts = allProducts.length > _displayBatchSize && _produkList.length < _maxDisplayProducts; // Show Load More if more than 20 products
                              _isLoading = false;
                            });
                          }
                        } catch (e) {
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_hasMoreProducts && !_isLoading && selectedBrand == null && _searchQuery.isEmpty && _produkList.length < _maxDisplayProducts)
                              Padding(
                                ...
                                child: Center(
                                  child: _isLoadingMore
                                      ...
                                      : ElevatedButton(
                                          onPressed: () => _loadProduk(loadMore: true),
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color: isSelected
                  ? const Color(0xFF0041c3).withOpacity(0.4)
  ...
  Widget _buildEnhancedProductList(
    ...
    List<dynamic> produkList = List.from(_fullProductList); // Use full product list for sections
