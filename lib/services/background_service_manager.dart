...
class BackgroundServiceManager {
  ...
  static Future<void> _registerPeriodicTasks() async {
    // Order status check - every 30 minutes
  ...
  static Future<void> _sendLocationToServer(Position position) async {
    try {
      ...
      print('✅ [BackgroundServiceManager] Location synced: ${position.latitude}, ${position.longitude}');
    } catch (e) {
  ...
  static Future<void> _checkAndSendBirthdayNotifications() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final sentNotifications = <String>[]; // In production, use persistent storage
          ...
          if (cosTglLahir != null && cosTglLahir.toString().isNotEmpty && cosTglLahir != '0000-00-00') {
            try {
              ...
              if (birthMonth == currentMonth && birthDay == currentDay) {
                ...
                await _sendBirthdayNotificationBackground(customerName, customerId);
              }
            } catch (e) {
              // Skip invalid dates
            }
  ...
  static Future<void> _sendBirthdayNotificationBackground(String customerName, String customerId) async {
    try {
