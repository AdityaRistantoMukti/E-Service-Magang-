...
class _ServicePageState extends State<ServicePage> {
  ...
  Future<void> _handleSearch(String kode) async {
    ...
    try {
      ...
      final allowedStatuses = ['itemsubmitted', 'waitingapproval', 'waitingorder', 'repairing', 'completed'];
      final cosKodeOrder = allOrders.firstWhere(
        (o) => o['cos_kode']?.toString() == kode && allowedStatuses.contains(o['trans_status']?.toString().toLowerCase()),
        orElse: () => null,
      );

      debugPrint('CosKodeOrder found: ${cosKodeOrder != null}');
      if (cosKodeOrder != null) {
        debugPrint('CosKodeOrder details: cos_kode=${cosKodeOrder['cos_kode']}, trans_status=${cosKodeOrder['trans_status']}, trans_kode=${cosKodeOrder['trans_kode']}');
        order = cosKodeOrder;
      } else {
        // If not found, search by trans_kode
        final transKodeOrders = allOrders.where((o) => o['trans_kode']?.toString() == kode).toList();
        debugPrint('TransKodeOrders count: ${transKodeOrders.length}');
        if (transKodeOrders.isNotEmpty) {
          order = transKodeOrders.first;
          debugPrint('TransKodeOrder details: cos_kode=${order['cos_kode']}, trans_status=${order['trans_status']}, trans_kode=${order['trans_kode']}');
        } else {
          ...
          try {
            ...
            if (transaksiData.isNotEmpty && transaksiData['cos_kode'] == currentUserId) {
              // Create a mock order object for pickup
              order = {
                'cos_kode': transaksiData['cos_kode'],
                'trans_status': transaksiData['trans_status']?.toString().toLowerCase() ?? 'waiting',
                'trans_kode': kode,
              };
              debugPrint('Pickup order found in transaksi: $order');
            } else {
              debugPrint('No trans_kode match found in order_list or transaksi');
              CustomDialog.show(
                context: context,
                icon: Container(
                  padding: const EdgeInsets.all(16),
      ...
      debugPrint('Order cos_kode: $orderCosKode, currentUserId: $currentUserId');
