...
import 'package:azza_service/Others/checkout_poin.dart';
...
import 'package:azza_service/Service/cleaning_service.dart';
  ...
  Future<void> _claimVoucher(Voucher voucher) async {
    if (_isVoucherClaimed(voucher)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Voucher sudah diklaim')),
      );
      return;
    }
    ...
    try {
      ...
      final response = await ApiService.claimVoucher(customerId, voucher.voucherId);
      if (response['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Voucher ${voucher.voucherCode} berhasil diklaim!')),
        );
              ...
              children: [
                ...
                Align(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: voucher.isActive && !_isVoucherClaimed(voucher) ? () => _claimVoucher(voucher) : null,
                    child: Text(
                      voucher.isActive ? (_isVoucherClaimed(voucher) ? "Claimed" : "Claim") : "Expired",
                      style: const TextStyle(
                        color: Colors.white,
                        fontSize: 11,
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => CheckoutPoinPage(
                                  produk: {
